...
class SheetsManager:
    ...
    def log_payment_check(
        ...
        try:
            ...
            logger.info("Payment check logged to Google Sheets: row %s, user %s, method %s", row_index, user_id, method)
            return row_index
        except Exception as exc:
            logger.error("Failed to log payment check to Google Sheets: %s", exc, exc_info=True)
            return None
