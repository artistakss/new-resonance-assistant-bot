...
def format_payment_details(method: str, details: str) -> str:
    ...
    Для Kaspi Bank: номер карты в backticks (копируется), имя получателя - обычным текстом.
    Для остальных методов: весь текст в backticks.
    """
    if method == "Kaspi":
        # Парсим реквизиты Kaspi: номер карты и имя получателя
        lines = [line.strip() for line in details.split('\n') if line.strip()]
        
        # Ищем номер карты (строка, содержащая только цифры, возможно с пробелами)
        card_number = None
        name_lines = []
        
        for line in lines:
            # Убираем все пробелы и проверяем, состоит ли строка только из цифр
            digits_only = re.sub(r'\s+', '', line)
            if re.match(r'^\d+$', digits_only) and len(digits_only) >= 10:
                # Это похоже на номер карты
                card_number = line
            else:
                # Это имя получателя или другая информация
                name_lines.append(line)
        
        if card_number:
            # Форматируем: номер карты в backticks, имя - обычным текстом
            ...
            # Если не нашли номер карты, возвращаем как есть в backticks
            return f"`{details}`"
    else:
        # Для остальных методов - весь текст в backticks
        return f"`{details}`"
...
async def choose_method(call: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        # Разный текст подсказки для Kaspi и остальных методов
        if method == "Kaspi":
            hint_text = (
                "💡 *Как скопировать номер карты:*\n"
                "• Нажмите на номер карты выше (он выделен) — скопируется только номер\n"
                "• Или долго нажмите на всё сообщение — скопируется весь текст"
            )
        else:
            hint_text = "💡 *Нажмите на реквизиты выше, чтобы скопировать их*"
